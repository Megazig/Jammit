...
/* 0x806761c0 */
int GetSaveCRC(u8* ptr, int length, int hash, int not) {
	if(length)
		while(length--)
			hash = (hash<<8) ^ saveTable[(*ptr++^(hash>>24))&0xff];
	if(not)
		hash = ~hash;
	return hash;
}
...
int main(int argc, char ** argv) {
	...
	u32 read = fread(buffer, 1, length, fp);
	fclose(fp);
	if ( read != length )
		return EXIT_FAILURE;
	printf("Read %d bytes from %s\n", length, argv[1]);

	u32 HeaderCRC = Read32(buffer + 0x40);
	u32 DataCRC = Read32(buffer + 0x44);
	Write32(buffer + 0x40, 0);
